...
if gpus:
    try:
        # فقط کارت اولو انتخاب کن
        tf.config.set_visible_devices(gpus[0], 'GPU')
        # بذار حداکثر نصف رمش رو استفاده کنه
        tf.config.set_logical_device_configuration(
            gpus[0],
            [tf.config.LogicalDeviceConfiguration(memory_limit=2048)]  # بر حسب مگابایت (مثلاً نصف ۸ گیگ)
        )
...
df = pd.read_csv(
    r'prerequisite\Run on CPU GPU\19 tir CPU\XAUUSD M1 2025-01-06 to 2025-04-17.csv',
    sep='\t',
    header=None,
...
## ================سیو کردنم  مدل ===================
model.save("my_model.h5")  # یا my_model.keras (در نسخه‌های جدید)
